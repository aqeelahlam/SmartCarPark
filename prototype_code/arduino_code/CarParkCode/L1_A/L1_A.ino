...
/*
  Arduino Controlling Slots L1_A1, L1_A2, L1_A3, L1_A4
...
Servo servo_entrance;   // Initialize a Servo Object for the connected servo      
   
// Initialize the angle
int angle = 0;    

// Ultrasonic Sensor L1_A4
...
// Ultrasonic Sensor for ENTRANCE GATE
...
void setup() {
  ...
  */
  // Ultrasonic Sensor for Slot L1_A4
  pinMode(trigPin1, OUTPUT);  
  pinMode(echoPin1, INPUT);   
  // Ultrasonic Sensor for Slot L1_A3
  pinMode(trigPin2, OUTPUT);
  pinMode(echoPin2, INPUT);
  // Ultrasonic Sensor for Slot L1_A2
  pinMode(trigPin3, OUTPUT);
  pinMode(echoPin3, INPUT);
  // Ultrasonic Sensor for Slot L1_A1
  ...
  servo_entrance.write(0);    // Sets the initial servo position to 0 (Horizontal)
...
void loop() {
  ...
  if(distanceG > 0 && distanceG <= 5) {
    ...
    for(angle = 0; angle < 90; angle += 1){   // Command to move from 0 degrees to 90 degrees                               
      servo_entrance.write(angle);                // Command to rotate the servo to the specified angle
      delay(15);                              // Delay for 15ms
    }
    servo_entrance.detach();                      // This will stop the servo being in loop
  }
  // Closing the Gate:
  else if(distanceG > 3) {    
    servo_entrance.attach(10);
    servo_entrance.write(0);
  }

/* 
   * Ultrasonic Sensor For Parking Slot L1_A4
   */
  ...
  /* 
   * Ultrasonic Sensor Parking Slot L1_A3
   */
  ...
  /* 
   * Ultrasonic Sensor For Parking Slot L1_A2
   */
  ...
  /* 
   * Ultrasonic Sensor For Parking Slot L1_A1
   */
